...
export const getContacts =
  ...
  async (dispatch) => {
    ...
    // const response = await axios.get('https://us-central1-mt10-hydro-detection-fbe3d.cloudfunctions.net/api/contacts', { params: { term, sortBy, pageSize, pageIndex } });
    const response = await axios.get('http://localhost:5001/mt10-hydro-detection-fbe3d/us-central1/api/contacts', { params: { term, sortBy, pageSize, pageIndex } });
    
    console.log(response);
    console.log(response.data);
    const contacts =  response.data.slice(0, 2);
    const pageCount = Math.ceil(contacts.length / pageSize);
