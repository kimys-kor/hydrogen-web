...
const ChartLargeLineSales = (props) => {
  const [device, setdevice] = useState(props)
  const avg = device.device.value.reduce( (a, c) => a + c ) / device.device.value.length
  ...
  return (
    <>
      <Col xs="12" sm="auto" className="d-flex flex-column justify-content-between custom-tooltip pe-0 pe-sm-4">
        <p className="heading title mb-1">기기Id: {device.device.deviceId}</p>
        <div ref={tooltipRef} />
        <Row>
          <Col xs="auto">
            <div className="cta-3 text-alternate">{avg}</div>
